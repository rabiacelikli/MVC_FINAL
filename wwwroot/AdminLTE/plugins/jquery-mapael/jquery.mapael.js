...
(function (factory) {
    ...
    Mapael.prototype = {
         ...
         *  opt.hiddenOpaPhotoFile opaPhotoFile for hidden element (default = 0.3)
        ...
        onShowElementsInRange: function(e, opt) {
            ...
            // set hiddenOpaPhotoFile to default if not defined
            if (opt.hiddenOpaPhotoFile === undefined) {
                opt.hiddenOpaPhotoFile = 0.3;
            }

            // handle area
            if (opt.ranges && opt.ranges.area) {
                self.showElemByRange(opt.ranges.area, self.areas, opt.hiddenOpaPhotoFile, opt.animDuration);
            }

            // handle plot
            if (opt.ranges && opt.ranges.plot) {
                self.showElemByRange(opt.ranges.plot, self.plots, opt.hiddenOpaPhotoFile, opt.animDuration);
            }

            // handle link
            if (opt.ranges && opt.ranges.link) {
                self.showElemByRange(opt.ranges.link, self.links, opt.hiddenOpaPhotoFile, opt.animDuration);
            }
         ...
         * @hiddenOpaPhotoFile: the opaPhotoFile when hidden
         * @animDuration: the animation duration
         */
        showElemByRange: function(ranges, elems, hiddenOpaPhotoFile, animDuration) {
            var self = this;
            // Hold the final opaPhotoFile value for all elements consolidated after applying each ranges
            // This allow to set the opaPhotoFile only once for each elements
            var elemsFinalOpaPhotoFile = {};
            ...
            $.each(ranges, function (valueIndex) {
                ...
                $.each(elems, function (id) {
                    ...
                    if ((range.min !== undefined && elemValue[valueIndex] < range.min) ||
                        ...
                        elemsFinalOpaPhotoFile[id] = hiddenOpaPhotoFile;
                    } else {
                        // Element in range
                        elemsFinalOpaPhotoFile[id] = 1;
                    }
                });
            });
            // Now that we looped through all ranges, we can really assign the final opaPhotoFile
            $.each(elemsFinalOpaPhotoFile, function (id) {
                self.setElementOpaPhotoFile(elems[id], elemsFinalOpaPhotoFile[id], animDuration);
            });
        },

        /*
         * Set element opaPhotoFile
         * Handle elem.mapElem and elem.textElem
         * @param elem the element
         * @param opaPhotoFile the opaPhotoFile to apply
         * @param animDuration the animation duration to use
         */
        setElementOpaPhotoFile: function(elem, opaPhotoFile, animDuration) {
            var self = this;
            ...
            // If final opaPhotoFile is not null, ensure element is shown before proceeding
            if (opaPhotoFile > 0) {
                elem.mapElem.show();
                if (elem.textElem) elem.textElem.show();
            }

            self.animate(elem.mapElem, {"opaPhotoFile": opaPhotoFile}, animDuration, function () {
                // If final attribute is 0, hide
                if (opaPhotoFile === 0) elem.mapElem.hide();
            });

            self.animate(elem.textElem, {"opaPhotoFile": opaPhotoFile}, animDuration, function () {
                // If final attribute is 0, hide
                if (opaPhotoFile === 0) elem.textElem.hide();
            });
        },
        ...
        onUpdateEvent: function (e, opt) {
            ...
            var fnRemoveElement = function (elem) {
                ...
                self.animate(elem.mapElem, {"opaPhotoFile": 0}, animDuration, function () {
                    elem.mapElem.remove();
                });

                self.animate(elem.textElem, {"opaPhotoFile": 0}, animDuration, function () {
            ...
            var fnShowElement = function (elem) {
                ...
                elem.mapElem.attr({opaPhotoFile: 0});
                if (elem.textElem) elem.textElem.attr({opaPhotoFile: 0});
                // Set final element opaPhotoFile
                self.setElementOpaPhotoFile(
                    elem,
                    (elem.mapElem.originalAttrs.opaPhotoFile !== undefined) ? elem.mapElem.originalAttrs.opaPhotoFile : 1,
                    animDuration
                );
            };
        ...
        handleClickOnLegendElem: function(elem, id, legendIndex, legendType, opts) {
            ...
            if (hidden === '0') {
                self.animate(legendLabel, {"opaPhotoFile": 0.5}, animDuration);
            } else {
                self.animate(legendLabel, {"opaPhotoFile": 1}, animDuration);
            }
            ...
            $.each(mapElems, function (y) {
                ...
                if (self.getLegendSlice(elemValue, legendOptions) === sliceOptions) {
                    if (hidden === '0') { // we want to hide this element
                        ...
                        self.setElementOpaPhotoFile(mapElems[y], legendOptions.hideElemsOnClick.opaPhotoFile, animDuration);
                        ...
                        if ($.isEmptyObject(hiddenBy)) {
                            self.setElementOpaPhotoFile(
                                mapElems[y],
                                mapElems[y].mapElem.originalAttrs.opaPhotoFile !== undefined ? mapElems[y].mapElem.originalAttrs.opaPhotoFile : 1,
                                animDuration
                            );
                        }
                    }
